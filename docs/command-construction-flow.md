...
aish> find all python files larger than 1mb modified in the last week█
...
aish> (docker run -v ...) mount current dir as /app and run node█
...
aish> (awk '{print $3}' ...) only include lines from today's date█
...
ZLE widget activates
  ├── Saves current $BUFFER to $AISH_SAVED_BUFFER
  ├── Replaces prompt with "aish> "
...
| Inline prompt vs popup | Inline (`aish>` replaces `$`) | Stays in flow, no mode switch |
...
- **Daemon unreachable** → show "aish: daemon not running, start with `aish start`", restore buffer
- **Timeout** → show "aish: timed out", restore buffer
- **Network failure** → 1 retry with 500ms delay. If both attempts fail, show "aish: couldn't reach API — check your connection", restore buffer
- **Circuit breaker open (offline)** → instant error: "aish: offline", restore buffer. No network call, no wait.
